...
data_source = st.sidebar.radio(
    ...
    help="Choose between fetching data from Yahoo Finance or uploading your own CSV file"
)
...
if data_source == "Yahoo Finance":
    ...
    ticker = st.sidebar.text_input(
        "Ticker Symbol",
        value="AAPL",
        help="Enter stock ticker symbol (e.g., AAPL, TSLA, GOOGL)"
    ).upper()
...
strategy_type = st.sidebar.selectbox(
    "Choose Strategy",
    ["SMA Crossover", "EMA Crossover", "RSI", "MACD", "Bollinger Bands", "Stochastic", "Williams %R"],
...
if strategy_type == "SMA Crossover":
    strategy_params['short_window'] = st.sidebar.slider("Short SMA Period", 5, 50, 10)
    strategy_params['long_window'] = st.sidebar.slider("Long SMA Period", 20, 200, 50)
    
elif strategy_type == "EMA Crossover":
    strategy_params['short_window'] = st.sidebar.slider("Short EMA Period", 5, 50, 12)
    strategy_params['long_window'] = st.sidebar.slider("Long EMA Period", 20, 200, 26)
    
elif strategy_type == "RSI":
    strategy_params['rsi_period'] = st.sidebar.slider("RSI Period", 5, 50, 14)
    strategy_params['rsi_overbought'] = st.sidebar.slider("RSI Overbought", 60, 90, 70)
    strategy_params['rsi_oversold'] = st.sidebar.slider("RSI Oversold", 10, 40, 30)
    ...
    strategy_params['signal_period'] = st.sidebar.slider("Signal Period", 5, 20, 9)
    
elif strategy_type == "Bollinger Bands":
    strategy_params['bb_period'] = st.sidebar.slider("BB Period", 10, 50, 20)
    strategy_params['bb_std'] = st.sidebar.slider("BB Standard Deviation", 1.0, 3.0, 2.0, 0.1)
    ...
    strategy_params['overbought'] = st.sidebar.slider("Overbought Level", 70, 90, 80)
    strategy_params['oversold'] = st.sidebar.slider("Oversold Level", 10, 30, 20)
    
elif strategy_type == "Williams %R":
    strategy_params['wr_period'] = st.sidebar.slider("Williams %R Period", 5, 30, 14)
    strategy_params['wr_overbought'] = st.sidebar.slider("Overbought Level", -30, -10, -20)
    strategy_params['wr_oversold'] = st.sidebar.slider("Oversold Level", -90, -70, -80)
...
if st.session_state.backtest_results:
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Sharpe Ratio", f"{sharpe_ratio:.3f}" if sharpe_ratio else "N/A")
    with col2:
        st.metric("Max Drawdown", f"{max_drawdown:.2f}%" if max_drawdown else "N/A")
    with col3:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Equity Curve", "📋 Trade Summary", "🔍 Risk Analysis", "💼 Portfolio Metrics"])
    ...
    with tab3:
        ...
        risk_metrics = risk_manager.calculate_risk_metrics(results_data['data']['Close'])
        
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Annual Volatility", f"{risk_metrics.get('volatility', 0):.2f}%")
        ...
        with col2:
            ...
            st.metric("Maximum Drawdown Duration", f"{risk_metrics.get('max_dd_duration', 0)} days")
        
        # Risk-return scatter plot
        risk_return_fig = risk_manager.create_risk_return_plot(results_data['data']['Close'])
        if risk_return_fig:
    ...
    with tab4:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Portfolio Beta", f"{portfolio_metrics.get('beta', 0):.3f}")
            st.metric("Information Ratio", f"{portfolio_metrics.get('information_ratio', 0):.3f}")
            st.metric("Treynor Ratio", f"{portfolio_metrics.get('treynor_ratio', 0):.3f}")
        
        with col2:
            st.metric("Jensen's Alpha", f"{portfolio_metrics.get('alpha', 0):.3f}")
            st.metric("Tracking Error", f"{portfolio_metrics.get('tracking_error', 0):.2f}%")
            st.metric("R-Squared", f"{portfolio_metrics.get('r_squared', 0):.3f}")

else:
    st.info("Please configure your data source and parameters in the sidebar, then run the backtest.")
    ...
    if data_source == "Upload CSV":
        ...
        st.info("Your CSV file should contain these exact column names: Date, Open, High, Low, Close, Volume")
